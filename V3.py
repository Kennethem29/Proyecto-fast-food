...
def mostrar_menu_principal():
    ...
    def ver_carrito():
        ...
        tk.Button(carrito_ventana, text="Pagar", command=lambda: pagar(total, carrito_ventana)).pack(pady=10)

    def pagar(total, ventana):
        if total == 0:
            messagebox.showinfo("Pagar", "El carrito está vacío.")
            return

        messagebox.showinfo("Pagar", f"Gracias por tu compra. Total pagado: $ {total}")
        carrito.clear()
        ventana.destroy()
